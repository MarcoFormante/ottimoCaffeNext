...
import { findText, parseToFloatFixedTwo } from "../utils/helpers/function";
             ...
             return (
                 <article id="art-1" key={product.id + product.price } className='product-card drop-shadow-lg'>
                     <figure className="flex flex-col justify-between h-full">
                        ...
                        <figcaption>
                            <h3 className="font-normal text-black">Ottimo Caffè - <span dangerouslySetInnerHTML={{ __html: findText(product.name,search) || ""}}/></h3>
                            <p className="product-card__desc font-normal mt-3 text-base"><span dangerouslySetInnerHTML={{ __html:findText(product.description,search) || "" }}/></p>
                            <p>
