    ...
    return (
        <div hidden={!totalProducts} className="flex justify-between items-center px-4 py-3">
                    <div className="text-sm text-slate-500">
                        <p>Pagina {page} di {Math.floor(pagination + 1)} /  <b>{totalProducts}</b> prodotti totali</p>
                    </div>
                    <div className="flex space-x-1">
                        ...
                        {pagination >= 0 && Array.from({length: pagination + 1 + (hasMoreThenTenProducts ? 1 : 0) }).map((_,i)=>{
                            const pageNumber = i 
                            return pageNumber !== 0 && (
                                <button 
                                key={pageNumber} 
